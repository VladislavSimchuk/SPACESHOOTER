...
class Bot(Sprite):
    ...
    def move(self, window, hero):
        ...
        if self.SHOT % (setting_win["FPS"] * 2) == 0:
            bullet_bot_boss_list.append(Bulled(self.centerx, self.bottom, 10, 20, color= (20,240,20)))
...
class Boss(Sprite):
    ...
    def leave(self, hero):
        ...
        if self.bottom < 0:
            ...
            self.x = setting_win["WIDTH"] // - setting_hero["WIDTH"] // 2
            self.y = - setting_boss["HEIGHT"]
            self.LIVE = False


class Bulled(pygame.Rect):
